 ...
 * 目标迁入回执保留清理调度器。
 */
public final class AgentMigrationTargetReceiptRetentionScheduler implements AutoCloseable {
    private final AgentMigrationTargetReceiptRetentionService retentionService;
    ...
    public AgentMigrationTargetReceiptRetentionScheduler(
            AgentMigrationTargetReceiptRetentionService retentionService,
            Duration interval
    ) {
        this(retentionService, interval, Executors.newSingleThreadScheduledExecutor(runnable -> {
            Thread thread = new Thread(runnable, "agent-migration-target-receipt-retention");
    ...
    private AgentMigrationTargetReceiptRetentionScheduler(
            AgentMigrationTargetReceiptRetentionService retentionService,
            Duration interval,
            ScheduledExecutorService scheduler,
    ...
    private void purgeSafely() {
        try {
            ...
            // 失败已记录在 retentionService 统计里，调度线程继续保活。
        }
    }
